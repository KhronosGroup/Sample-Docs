...
Disadvantages of GFA are:
  ...
  * Cannot generate PDF in the browser view, only live HTML.
...
Disadvantages of extended Asciidoctor are:
  ...
  * Requires learning a small amount of Ruby, and about Asciidoctor
    internals.
    Their internal documentation has been improving, but you may still in
    some cases need to ask questions on the
    https://asciidoctor.zulipchat.com/[Asciidoctor support forum], or read
    the Asciidoctor source code.
...
for Asciidoc and Asciidoctor, and is also widely used.
...
is Khronos' first use of Antora.
It incorporates the Vulkan and GLSL Specifications as well as a large amount
of developer-oriented material including the Vulkan Guide, Reference Pages,
Samples, and Tutorial.

Several other Khronos Working Groups are beginning a transition to using
Antora, and over time, we may publish most of our specifications through
Antora instead of the <<repomanagement-registry, Khronos Registry>>.
However, we are still learning, are not ready to recommend Antora as the
default publishing format yet.
The Sample repository does not yet demonstrate use of Antora.
...
Advantages of Antora:
  ...
  * Enables unified documentation websites combining specifications,
    ...
    ecosystem.
  * Site-wide search using the Lunr extension.
...
transform Vulkan and GLSL specification markup into an Antora-friendly form.
...
A similar approach is used with other components of the Vulkan Documentation
Website, such as the Vulkan Guide, Tutorial, and Samples, but the details
differ because they use different markup conventions.
The Vulkan conversion scripts may be applicable to some other repositories
already using asciidoctor as well, or we can provide assistance in
constructing a repository-specific conversion tool.
