...
This chapter incorporates and supersedes the
link:https://members.khronos.org/wg/OpenCL/mail/thread/45534#60056["`Khronos
Open Source Repository Resources`"] document in the member Causeway area.
...
The Khronos default licenses for specifications are:
  ...
  * Published specification artifacts - generated files such as HTML and PDF
    ...
    The Khronos Specification Copyright License should never be used on
...
copyright statement in the format described above:
  ...
  . _Externally Sourced_ files come from other open source projects.
    ...
    or implicitly using the LICENSE file of their source repository to apply
...
To provide a license for these files, add them to the `REUSE.toml` file
...
the file `REUSE.toml`.
The https://reuse.software/spec-3.3/#reusetoml[file syntax] is documented on
the REUSE website if the examples in this repository are not sufficient.

Some examples of the `REUSE.toml` syntax for a file or group of files are:

[source]
----
[[annotations]]
path = ["images/**.svg", "config/docinfo-header.html"]
...
The `path` line contains a comma-separated list of names of one or more
files in the repository.
Wildcards are allowed, including `pass:[**]` to match path separators.

The `SPDX-FileCopyrightText` line contains the copyright statement for those
files.
If there is already a copyright statement in the file without a
corresponding license, it should be copied verbatim.

The `SPDX-License-{empty}Identifier` line contains the SPDX license
identifier of those files.

Multiple sections may be added to `REUSE.toml` for different copyrights or
licenses.
Be careful not to include filenames repeatedly, or include filenames which
already have explicit licenses in their text.
The `precedence` line allows some control over treatment of files with
multiple, inconsistent licenses.
