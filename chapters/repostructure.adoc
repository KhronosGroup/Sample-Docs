...
as follows:
  ...
  * `Makefile` (or other build script such as CMake or Meson) to build the
  ...
  * `sample.adoc` (or `__apiname__.adoc` or other meaningful name) -
  ...
  * `images/` - directory containing SVG images included in the
    specification.
    While other image file formats are supported, SVG is preferred as it can
    be used with good quality results in both HTML and PDF builds.
    This directory is *not* a notational convenience.
    Images and other files which are not themselves asciidoc markup must be
