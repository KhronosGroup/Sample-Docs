...
Khronos specifications are intended for use by both implementers, and
by application developers seeking to make use of implementations.
A specification forms a contract between these parties.

Specification text may address either implementers or developers.
...
In text addressing implementers, their use expresses requirements that apply
to implementations.
...
These key words are never used in text addressing implementers.
